...
| **Color space** | Controls which perceptual color model is used to convert L, C, h values into sRGB hex output. When any non-OKLCH space is active, the hue gradient and gamut ceiling overlay update to reflect that space's sRGB boundary. See [Comparing color spaces](docs/colorspaces.md) for guidance.<ul><li><strong>OKLCH</strong> (default) — OKLab (Björn Ottosson, 2020).</li><li><strong>SRLCH</strong> — SRLAB2 cylindrical coordinates (Jan Behrens, 2011), scaling L × 100 and C × 300 to SRLAB2 natural units.</li><li><strong>CIELCH</strong> — CIE L*C*h° (CIELAB cylindrical), scaling identically.</li><li><strong>LCHUV</strong> — CIELChUV, the cylindrical form of CIELUV — the same L* definition as CIELAB but with u*/v* chromatic axes derived from the CIE 1976 UCS diagram, also scaling L × 100 and C × 300.</li><li><strong>JzCzHz</strong> — cylindrical form of JzAzBz (Safdar et al., 2017), a modern HDR-capable space built on the ST 2084 PQ transfer function. Jz maps directly to the UI L range [0, 1] (D65 white ≈ 0.9999); Cz is scaled by 0.5 to match the sRGB gamut range.</li><li><strong>Munsell</strong> — the 1905 Munsell HVC system (A. H. Munsell), as measured in the 1943 Newhall-Nickerson-Judd renotation. The only empirically defined space in the tool: coordinates are derived from observer experiments, not a formula. L × 10 = Munsell Value [0, 10]; C × 60 = Munsell Chroma [0, ~24]; H° maps linearly to Munsell Hue [0, 100] (0° = 5R, 90° ≈ 5Y, 180° ≈ 5G, 270° ≈ 5B). Useful for matching physical Munsell color standards or cross-referencing against the perceptual ground truth that CIELAB and OKLab were designed to approximate.</li></ul> |
| **Illuminant** | Applies a Bradford chromatic adaptation transform to the linear RGB output, simulating how the palette appears under a different reference illuminant on a D65 display. The gamut ceiling on the canvas updates to reflect the adapted color volume. See [Comparing color spaces — Illuminant](docs/colorspaces.md#illuminant) for full details.<ul><li><strong>D50</strong> — 5000 K, the standard illuminant for graphic arts and print (ICC profiles, soft-proofing). Warm shift: reds and oranges intensified, blues pulled back.</li><li><strong>Standard D65</strong> (default) — 6500 K, the reference for sRGB and all CSS color functions. No adaptation applied.</li><li><strong>D75</strong> — 7500 K, overcast daylight / northern skylight. Cool shift: blues intensified, reds reduced.</li></ul> |
...
The canvas draws a **dashed white line** showing the maximum in-gamut sRGB chroma at each lightness value for the current hue. This ceiling is computed via binary search across the full L range (0–1) and updates live as you move the Hue, Hue Shift, or Illuminant controls. When Hue Shift is non-zero, each L position on the ceiling reflects the shifted hue that the corresponding ramp step would actually use. When a non-default Illuminant is active, the ceiling reflects the gamut boundary after the Bradford chromatic adaptation — so it accurately shows how much headroom remains in the adapted color volume, not the raw D65 gamut. The ceiling provides an at-a-glance read of how much headroom the chroma curve has before gamut mapping engages.

---

## Comparing color spaces

For a detailed history and analysis of each color space — OKLCH, CIELCH, LCHUV, SRLCH, JzCzHz, and Munsell — including their design rationale, known tradeoffs, and guidance on when to use each, see [docs/colorspaces.md](docs/colorspaces.md).

For the complete mathematical specification of the chroma curve — including parameter definitions, Q-adjusted knee formulas, Fritsch-Carlson spline construction, and gamut compression — see [docs/chroma-curve-math.md](docs/chroma-curve-math.md).
