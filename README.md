...
| **Soft Knee** | When **YES**, softens the hard chroma transition that forms where the gamut ceiling intersects the chroma curve. At this intersection, stops on one side are held at the ceiling while stops on the other side follow the natural curve descent — creating an abrupt step in the discrete ramp. Soft Knee detects the boundary stops (where clipping status transitions between clipped and non-clipped) and applies a weighted `[0.25, 0.5, 0.25]` blend to those stops and their immediate neighbors, smoothing the knee on both sides of the clipped region simultaneously. Works in both SMART and COMP gamut modes. When no stops are clipped, falls back to reducing the global chroma peak by blending it with its neighbors. Default **NO**. |
